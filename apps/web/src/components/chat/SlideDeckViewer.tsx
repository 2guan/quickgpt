...
const quoteBlockItem = (block: string): SlideItem => {
  const [title, ...description] = block.split('\n').filter(Boolean);
  return { title: cleanMarkdownText(title || '核心建议'), description: description.join('\n'), bullets: [] };
};
const LayeredCallout: React.FC<{ text: string; theme: SlideTheme; bold?: boolean }> = ({ text, theme, bold = false }) => <div className="relative h-9 shrink-0"><div className="absolute inset-y-0 -left-1 right-1 rounded-xl" style={{ backgroundColor: theme.bg }} /><aside className={`absolute inset-y-0 left-1 right-0 rounded-xl px-4 py-2 text-center text-[11px] whitespace-pre-line leading-relaxed dark:bg-slate-800 dark:text-slate-200 ${bold ? 'font-bold' : 'font-medium'}`} style={{ backgroundColor: theme.cardBg, color: theme.textColor }}>{renderFormattedText(text)}</aside></div>;
...
const chartSeries = (chart: NonNullable<SlideData['chart']>, seriesIndex: number) => { const series = chart.series[seriesIndex]; return { ...chart, title: series?.name || chart.title, series: series ? [series] : [] }; };
const MiniCard: React.FC<{ item: SlideItem; theme: SlideTheme; colorful: boolean; index: number }> = ({ item, theme, colorful, index }) => <ItemCard item={item} index={index} theme={theme} compact colorful={colorful} />;
const ChallengeRow: React.FC<{ item: SlideItem; index: number; theme: SlideTheme; colorful: boolean }> = ({ item, index, theme, colorful }) => {
  const [problem, solution] = item.children || [];
  const label = { ...item, description: '', bullets: [] };
  return <div className="grid min-h-0 grid-cols-[1fr_2fr_2fr] gap-3"><MiniCard item={label} index={index} theme={theme} colorful={colorful} /><MiniCard item={problem || { title: '现状与难点', description: itemText(item) }} index={index} theme={theme} colorful={colorful} /><MiniCard item={solution || { title: '对应方案', description: '请补充解决方案' }} index={index + 1} theme={theme} colorful={colorful} /></div>;
};
...
const SlideBody: React.FC<{ slide: SlideData; theme: SlideTheme; colorful: boolean }> = ({ slide, theme, colorful }) => {
  ...
  if (plan.kind === 'quote') { const quoteBlocks = slide.quoteBlocks || []; if (quoteBlocks.length > 1) { const rules = quoteBlocks.slice(0, 3); const footer = quoteBlocks.slice(3).join('\n'); return <div className="flex min-h-0 flex-1 flex-col justify-center gap-3"><div className={`grid gap-3 ${rules.length === 1 ? 'grid-cols-1' : rules.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>{rules.map((block, index) => <ItemCard key={index} item={quoteBlockItem(block)} index={index} theme={theme} compact colorful={colorful} />)}</div>{items.length > 0 && <div className={`grid min-h-0 flex-1 gap-3 ${items.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>{items.map((item, index) => <ItemCard key={index} item={item} index={index + rules.length} theme={theme} compact colorful={colorful} />)}</div>}{footer && <LayeredCallout text={footer} theme={theme} />}{slide.notes && <LayeredCallout text={`💡 ${slide.notes}`} theme={theme} />}</div>; } return <div className="flex flex-1 flex-col justify-center gap-5"><blockquote className="m-0 whitespace-pre-line rounded-2xl border-l-8 bg-slate-50 px-8 py-7 text-center text-[22px] font-bold leading-relaxed text-slate-800 shadow-sm dark:bg-slate-800 dark:text-slate-100" style={{ borderLeftColor: theme.accent }}>{renderFormattedText(slide.quoteText || slide.title)}</blockquote>{items.length > 0 && <div className={`grid gap-3 ${items.length === 1 ? 'grid-cols-1' : items.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>{items.map((item, index) => <ItemCard key={index} item={item} index={index} theme={theme} compact colorful={colorful} />)}</div>}{slide.notes && <LayeredCallout text={`💡 ${slide.notes}`} theme={theme} />}</div>; }
  ...
  if (plan.kind === 'challenge') return <div className="flex min-h-0 flex-1 flex-col justify-center gap-3">{items.slice(0, 4).map((item, index) => <ChallengeRow key={index} item={item} index={index} theme={theme} colorful={colorful} />)}{slide.quoteText && <LayeredCallout text={slide.quoteText} theme={theme} bold />}</div>;
  if (plan.kind === 'hub') { const positions = [['2', '1'], ['1', '1'], ['3', '1'], ['1', '3'], ['3', '3'], ['2', '3']]; return <div className="relative grid min-h-0 flex-1 grid-cols-3 grid-rows-3 gap-3">{items.slice(0, 6).map((item, index) => <div key={index} className="min-h-0" style={{ gridColumn: positions[index][0], gridRow: positions[index][1] }}><MiniCard item={item} index={index} theme={theme} colorful={colorful} /></div>)}<div className="z-10 col-start-2 row-start-2 flex items-center justify-center"><div className="flex h-28 w-28 rotate-45 items-center justify-center rounded-2xl text-center shadow-lg" style={{ backgroundColor: theme.accent }}><p className="-rotate-45 px-2 text-sm font-bold leading-snug text-white">{renderFormattedText(slide.subtitle || slide.title)}</p></div></div></div>; }
  if (plan.kind === 'chartGrid' && slide.chart) { const series = slide.chart.series.slice(0, 4); return <div className="flex min-h-0 flex-1 flex-col justify-center gap-3"><div className={`grid min-h-0 flex-1 gap-3 ${series.length <= 2 ? 'grid-cols-2' : 'grid-cols-4'}`}>{series.map((_, index) => <ChartView key={index} chart={chartSeries(slide.chart!, index)} theme={theme} colorful={colorful} />)}</div>{items.length > 0 && <div className={`grid gap-3 ${items.length === 1 ? 'grid-cols-1' : items.length === 2 ? 'grid-cols-2' : 'grid-cols-4'}`}>{items.slice(0, 4).map((item, index) => <MiniCard key={index} item={item} index={index} theme={theme} colorful={colorful} />)}</div>}</div>; }
  if (plan.kind === 'dashboard' && slide.chart) { const series = slide.chart.series.slice(0, 2); return <div className="flex min-h-0 flex-1 flex-col gap-3"><div className={`grid gap-3 ${items.length <= 2 ? 'grid-cols-2' : 'grid-cols-4'}`}>{items.slice(0, 4).map((item, index) => <MiniCard key={index} item={item} index={index} theme={theme} colorful={colorful} />)}</div><div className={`grid min-h-0 flex-1 gap-4 ${series.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>{series.map((_, index) => <ChartView key={index} chart={chartSeries(slide.chart!, index)} theme={theme} colorful={colorful} />)}</div></div>; }
...
function addPptLayeredCallout(slide: any, deck: any, text: string, theme: SlideTheme, isDark: boolean, y: number, h: number, bold = false) {
  const colors = pptColors(isDark);
  slide.addShape(deck.ShapeType.roundRect, { x: 0.54, y, w: 8.8, h, rectRadius: 0.06, fill: { color: hex(theme.bg) }, line: { color: hex(theme.bg), transparency: 100 } });
  slide.addShape(deck.ShapeType.roundRect, { x: 0.6, y, w: 8.8, h, rectRadius: 0.06, fill: { color: isDark ? colors.card : hex(theme.cardBg) }, line: { color: isDark ? colors.card : hex(theme.cardBg), transparency: 100 } });
  slide.addText(cleanMarkdownText(text), { x: 0.76, y: y + 0.07, w: 8.45, h: h - 0.12, ...pptText(h < 0.4 ? 8.2 : 8.5, isDark ? colors.body : hex(theme.textColor), bold), align: 'center', valign: 'middle' });
}
...
function addPptHub(slide: any, deck: any, data: SlideData, theme: SlideTheme, isDark: boolean, colorful: boolean, contentY: number, contentH: number) {
  const items = slideItems(data).slice(0, 6); const positions = [[3.55, contentY, 2.9, 0.78], [0.6, contentY + 0.72, 2.65, 1.0], [6.75, contentY + 0.72, 2.65, 1.0], [0.6, contentY + 2.2, 2.65, 1.0], [6.75, contentY + 2.2, 2.65, 1.0], [3.55, contentY + 3.0, 2.9, 0.78]];
  items.forEach((item, index) => { const [x, y, w, h] = positions[index]; addPptCard(slide, deck, item, index, x, y, w, h, theme, false, isDark, colorful); });
  slide.addShape(deck.ShapeType.diamond, { x: 4.15, y: contentY + 1.63, w: 1.7, h: 1.7, fill: { color: hex(theme.accent) }, line: { color: hex(theme.accent) } });
  slide.addText(cleanMarkdownText(data.subtitle || data.title), { x: 4.35, y: contentY + 2.08, w: 1.3, h: 0.7, fontFace: 'Microsoft YaHei', fontSize: 10, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', fit: 'shrink' as const, margin: 0.03 });
}
function addPptChallenge(slide: any, deck: any, data: SlideData, theme: SlideTheme, isDark: boolean, colorful: boolean, contentY: number, contentH: number) {
  const items = slideItems(data).slice(0, 4); const gap = PPT_CARD_GAP; const rowH = (contentH - gap * Math.max(0, items.length - 1)) / Math.max(1, items.length);
  items.forEach((item, index) => { const y = contentY + index * (rowH + gap); const [problem, solution] = item.children || []; addPptCard(slide, deck, { ...item, description: '', bullets: [] }, index, 0.6, y, 1.65, rowH, theme, false, isDark, colorful); addPptCard(slide, deck, problem || { title: '现状与难点', description: itemText(item) }, index, 2.45, y, 3.35, rowH, theme, false, isDark, colorful); addPptCard(slide, deck, solution || { title: '对应方案', description: '请补充解决方案' }, index + 1, 6.0, y, 3.4, rowH, theme, false, isDark, colorful); });
}
function addPptChartGrid(slide: any, deck: any, data: SlideData, theme: SlideTheme, isDark: boolean, colorful: boolean, contentY: number, contentH: number) {
  const chart = data.chart!; const series = chart.series.slice(0, 4); const gap = 0.15; const columns = series.length <= 2 ? 2 : 4; const chartW = (8.8 - gap * (columns - 1)) / columns; const insightH = data.items.length ? 0.82 : 0; const chartH = contentH - insightH - (insightH ? gap : 0);
  series.forEach((_, index) => addPptChart(slide, deck, chartSeries(chart, index), theme, isDark, colorful, 0.6 + index * (chartW + gap), contentY, chartW, chartH));
  if (data.items.length) { const insightW = (8.8 - gap * (Math.min(4, data.items.length) - 1)) / Math.min(4, data.items.length); data.items.slice(0, 4).forEach((item, index) => addPptCard(slide, deck, item, index, 0.6 + index * (insightW + gap), contentY + chartH + gap, insightW, insightH, theme, false, isDark, colorful)); }
}
function addPptDashboard(slide: any, deck: any, data: SlideData, theme: SlideTheme, isDark: boolean, colorful: boolean, contentY: number, contentH: number) {
  const items = slideItems(data).slice(0, 4); const gap = PPT_CARD_GAP; const metricH = items.length ? 0.72 : 0; const metricW = (8.8 - gap * Math.max(0, items.length - 1)) / Math.max(1, items.length); items.forEach((item, index) => addPptCard(slide, deck, item, index, 0.6 + index * (metricW + gap), contentY, metricW, metricH, theme, false, isDark, colorful));
  const charts = data.chart!.series.slice(0, 2); const chartY = contentY + metricH + (metricH ? gap : 0); const chartH = contentH - metricH - (metricH ? gap : 0); const chartW = (8.8 - gap * Math.max(0, charts.length - 1)) / Math.max(1, charts.length); charts.forEach((_, index) => addPptChart(slide, deck, chartSeries(data.chart!, index), theme, isDark, colorful, 0.6 + index * (chartW + gap), chartY, chartW, chartH));
}
async function exportPptx(slides: SlideData[], theme: SlideTheme, isDark: boolean, colorful: boolean) {
  ...
  slides.forEach((data) => {
    ...
    else if (plan.kind === 'hub') addPptHub(slide, deck, data, theme, isDark, colorful, contentY, contentH);
    else if (plan.kind === 'challenge') addPptChallenge(slide, deck, data, theme, isDark, colorful, contentY, contentH);
    else if (plan.kind === 'chartGrid' && data.chart) addPptChartGrid(slide, deck, data, theme, isDark, colorful, contentY, contentH);
    else if (plan.kind === 'dashboard' && data.chart) addPptDashboard(slide, deck, data, theme, isDark, colorful, contentY, contentH);
...
export const SlideDeckViewer: React.FC<SlideDeckProps> = ({ rawCode, isStreaming = false }) => {
  ...
  useEffect(() => {
    // A partial page has no stable height, so it must not enter the fixed 16:9 canvas.
    setSlides(buildSlideDeck(isStreaming ? completedPptMarkdown(rawCode) : rawCode));
  }, [rawCode, isStreaming]);
  useEffect(() => {
  ...
  const deckTitle = useMemo(() => {
    const raw = slides[0]?.title || '';
    const cleaned = cleanMarkdownText(raw);
    return cleaned && cleaned !== '演示文稿' ? cleaned : '演示文稿';
  }, [slides]);
  ...
  const download = async () => { if (!slides.length || isStreaming) return; setIsExporting(true); try { await exportPptx(slides, theme, isDark, isColorful); } catch (error: any) { alert(`导出 PPTX 失败：${error.message}`); } finally { setIsExporting(false); } };
